        ...
        <div className="px-6 py-6">
          {step === 'success' ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Enjoy unlimited documents, custom branding, bulk export, and priority templates.</p>
            </div>
          ) : (
            <>
              {/* Plan summary */}
              <div className="mb-6 rounded-xl bg-indigo-50 border border-indigo-100 p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-indigo-600 font-medium">LegalKit Pro</div>
                ...
                <>
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-gray-200 px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                    />
                  </div>
                  {error && (
              ...
              {step === 'pending' && (
                <>
                  <div className="mb-6 space-y-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <code className="flex-1 rounded-lg bg-gray-50 border border-gray-200 px-3 py-2.5 text-xs text-gray-700 break-all font-mono">
                          {WALLET_ADDRESS}
                        </code>
                        <button
                          onClick={copyAddress}
                          className="shrink-0 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-xs text-gray-600 hover:bg-gray-100 transition-colors"
                        >
                          {copied ? '✓' : '📋'}
                        </button>
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="0x..."
                        className="w-full rounded-lg border border-gray-200 px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-indigo-500 focus:outline-none font-mono"
                      />
                    </div>
                  </div>
                  ...
                  <button
                    onClick={handleClaim}
                    className="w-full rounded-lg bg-green-600 py-3 text-sm font-semibold text-white hover:bg-green-500 transition-colors"
                  >
                    I&apos;ve Sent Payment ✓
                  </button>
                  <button
                    onClick={() => setStep('info')}
                    className="w-full mt-2 rounded-lg border border-gray-200 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors"
                  >
                    ← Back
                  </button>
                </>
